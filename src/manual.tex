...
Особенности реализации скриптов |nomake|:
\begin{itemize}
\item За один запуск обрабатывается только одна цель, что обусловлено ограничениями синтаксиса cmd-файлов.
\item Не реализовано отслеживание даты изменения исходных файлов.
\item Скрипты для обработки изображений\footnote{См.~|include/latex.fig.nmk.cmd|.} не перезаписывают существующие файлы во избежание потерь информации.
    Для пересборки необходимо удалять файлы вручную.
\end{itemize}
...
\item[clean]
  Удаление файлов, определенных в переменной |FIGCLFILES|.
  По умолчанию обрабатываются все файлы с расширениями pdf, jpg, png и tif.
\item[epstoeps]
  Оптимизация EPS-файлов.
  Следует применять только к изображениям с контурными шрифтами (outline fonts).
  Используемая программа: |eps2eps| из пакета Ghostscript.
