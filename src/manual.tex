...
Описание исходных текстов содержится в файлах \href{file:disser.pdf}{\file{disser.pdf}}, \href{file:disser-bst.pdf}{\file{disser-bst.pdf}} и \href{file:gost732.pdf}{\file{gost732.pdf}}.
\end{abstract}
...
В состав пакета входят классы \file{disser.cls} и \file{gost732.cls}, стили оформления библиографии \file{disser.bst}, \file{disser-s.bst}, а также скрипты для сборки пакета и документов (\file{Makefile}, \file{nomake.cmd}, файлы с расширением \file{mk} и \file{cmd}).
...
\\\hline
  ...
  Для использования опции необходимо установить пакеты \pkg{pscyr} (или \pkg{cyrtimes}) и \pkg{txfonts} (или \pkg{mtpro}).
\\\hline
...
\\\hline
  ...
  При наличии опции times вместо пакета \pkg{txfonts} подключается пакет \pkg{mtpro}.
\\\hline\hline
\end{tabularx}
...
\item[clean]
  Цель |clean| в каталогах \cmd{src} и \cmd{templates}.
\item[doc]
...
\item[install]
  ...
  Соответствует цели \cmd{install} в каталогах \file{disser/src} и \file{disser/templates}.
\item[package]
  Сборка пакета.
  Соответствует цели \cmd{package} в каталоге \file{disser/src}.
  Цель по умолчанию.
\item[reinstall]
  Переустановка пакета.
  Соответствует цели \cmd{reinstall} в каталоге \file{disser/src}.
\item[srcdist]
  Создание архива, содержащего исходные тексты пакета.
  Эквивалентно команде \cmd{hg archive}.
\item[tds]
  Создание TDS-архива, содержащего скомпилированные файлы пакета и шаблонов.
\item[templates]
  Сборка всех шаблонов.
  Соответствует цели по умолчанию в каталоге \file{disser/templates}.
\item[uninstall]
  Удаление файлов пакета из дерева каталогов \TeX{}.
  Соответствует целям \cmd{uninstall} в каталогах \file{disser/src} и \file{disser/templates}.
\end{deflist}
...
Для оформления библиографических ссылок используются стили \file{disser.bst} и \file{disser-s.bst}.
...
Стиль \file{disser-s.bst} аналогичен \file{disser.bst}, но сортирует записи в алфавитном порядке.

Основные отличия \file{disser.bst} от \file{gost71u.bst}:
\begin{enumerate}
...
В стилях \file{disser.bst} и \file{disser-s.bst} введены макросы |\BibPrefix| и |\BibPostfix|, позволяющие задать префикс и постфикс номера ссылки.
