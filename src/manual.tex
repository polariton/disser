...
Для достижения этой цели была переопределена часть команд, входящих в ядро \LaTeX{}, и введен ряд глобальных переменных, которые описаны в комментариях к исходным текстам.

В состав пакета входят классы |disser.cls| и |gost732.cls|, стили оформления библиографии |disser.bst|, |disser-s.bst|, а также скрипты для сборки пакета и шаблонов (|Makefile|, |nomake.cmd|).
...
Зависимости класса и шаблонов от сторонних пакетов:
|amsfonts|, |amsmath|, |amssymb|, |caption|, |cmap|, |graphicx|, |hyperref|, |hypcap|, |hypernat|, |ifpdf|, |multibib|, |natbib|, |pscyr| или |cyrtimes|, |txfonts| или |mtpro|, |subfig|.
...
\section{Кроссплатформенная система сборки документов}
...
В стиле |disser.bst| введены макросы |\BibPrefix| и |\BibPostfix|, позволяющие задать префикс и постфикс номера ссылки.
...
\item Настраиваемый формат номера библиографической ссылки для оформления списка работ диссертанта в автореферате.
\end{enumerate}

Для работы с русскоязычными библиографическими ссылками необходима версия \BibTeX{} с поддержкой 8-битных символов, |bibtex8|.
В зависимости от кодировки текста в bib-файле, необходимо подключать csf-файл с описанием кодовой страницы и порядка сортировки, например, |cp1251.csf|.
...
К сожалению, пакет |natbib| не <<сжимает>> ссылки с нецифровыми символами, поэтому ссылки вида [A1, A2, A3, A4] не преобразуются к более компактной форме [A1--A4].
