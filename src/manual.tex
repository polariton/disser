...
Для выделения курсивом некоторых полей библиографической записи, в файле \file{thesis.tex} нужно добавить определение
\begin{verbatim}
...
Код, определяющий вид титульного листа, нумерацию объектов и другие параметры, вынесен в отдельные файлы, имена которых совпадают с соответствующими опциями класса (см. раздел~\ref{s:opts}):
...
Для внесения изменений в файлы пакета (|bst|, |cls| и |rtx|) следует создавать их копии в каталоге с документом.
При сборке документа \LaTeX{} ищет подключаемые файлы в каталоге основного |tex|-файла, поэтому будет использована локальная копия файлов с вашими изменениями.
